include <mangopi mq-quad dimensions.scad>
use <../parts/feet.scad>
use <../parts/generic bracket.scad>
use <../parts/generic drilling templates.scad>
use <../parts/dimensions verifier.scad>
use <../../../Mobile Studio/hot shoe/files/hotshoe_adapter_v2.scad>
use <../utils/intersection.scad>
include <mangopi mq-quad label.scad>

/*
mangopi_mqquad_feet();
translate(size) */ mangopi_mqquad_bracket();
/*translate(size) translate(size) mangopi_mqquad_drilling_template();
translate(size) translate(size) translate(size) mangopi_mqquad_dimensions_verifier();
translate([- size.x, size.y, size.z]) translate(size) translate(size) translate(size) mangopi_mqquad_hotShoe_adapters();
translate([- size.x, size.y, size.z]) translate(size) translate(size) translate(size) translate(size)
    mangopi_mqquad_screw_on_plank();*/
/*translate([- size.x, size.y, size.z]) mangopi_mqquad_vertical_bracket();*/

module mangopi_mqquad_feet() {
    feet_feet(feet, holeSize, baseSize, baseHeight, totalHeight) ;
}

module mangopi_mqquad_bracket() {
    union() {
    bracket_bracket(feet, holeSize, baseSize, baseHeight, totalHeight, linkThickness, linkHeight);
    tanOppositeAngle=(feet[1].x-feet[0].x)/(feet[3].y-feet[1].y);
    echo("Tan is", tanOppositeAngle);
    oppositeAngle=90 - atan(tanOppositeAngle);
    echo("Angle is", oppositeAngle);
        diagonal = sqrt(feet[3].x^2+feet[3].y^2);
        labelLength = (diagonal / 2) - baseSize / 2 - linkThickness / 2;
    rotate([0,0,oppositeAngle]) 
        translate([baseSize/2,-baseSize*0.65,baseHeight-plateHeight])
                    drawLabel(label, baseSize*.6, labelLength, plateHeight, linkHeight/3, feet);
//drawLabel(label, baseSize*0.8, plateHeight, linkHeight, feet); 

    }
}

module mangopi_mqquad_drilling_template() {
    drillTemplate(feet, holeSize, drillTemplateThickness, drillTemplateGuideHeight);
}

module mangopi_mqquad_dimensions_verifier() {
    verifier_checkDimensions(feet, holeSize, verifierPlateThickness);
}

module mangopi_mqquad_hotShoe_adapters() {
    mangopi_mqquad_hotShoe_adapter_main();
    translate([- size.x * 2, 0, 0]) mangopi_mqquad_hotShoe_adapter_reverted();
    translate([- size.x * 3, 0, 0]) mangopi_mqquad_hotShoe_adapter_vertical();
    translate([- size.x * 4, 0, 0]) mangopi_mqquad_hotShoe_adapter_vertical_90_degrees();
}

module mangopi_mqquad_hotShoe_adapter_reverted() {
    mangopi_mqquad_hotShoe_adapter_base();
    middlePoint = IntersectionOfLines(feet[0], feet[3], feet[2], feet[1]);
    rotate(a = [0, 0, 180]) translate([0, 0, hotShoeHeightClearance - baseHeight]) translate(- middlePoint)
        mangopi_mqquad_bracket();
}

module mangopi_mqquad_hotShoe_adapter_main() {
    mangopi_mqquad_hotShoe_adapter_base();
    middlePoint = IntersectionOfLines(feet[0], feet[3], feet[2], feet[1]);
    translate([0, 0, hotShoeHeightClearance - baseHeight]) translate(- middlePoint) mangopi_mqquad_bracket();
}

module mangopi_mqquad_hotShoe_adapter_vertical() {
    mangopi_mqquad_hotShoe_adapter_base();
    middlePoint = IntersectionOfLines(feet[0], feet[3], feet[2], feet[1]);

    xMiddle = max(feet[0][0], feet[1][0], feet[2][0], feet[3][0]) / 2;
    // How much of the board will go past the X holes?
    xOverHang = abs(size.x - (xMiddle * 2));

    translate([1, 15, hotShoeHeightClearance * 2]) mangopi_mqquad_vertical_bracket();
}

module mangopi_mqquad_hotShoe_adapter_vertical_90_degrees() {
    mangopi_mqquad_hotShoe_adapter_base();
    middlePoint = IntersectionOfLines(feet[0], feet[3], feet[2], feet[1]);

    xMiddle = max(feet[0][0], feet[1][0], feet[2][0], feet[3][0]) / 2;
    // How much of the board will go past the X holes?
    xOverHang = abs(size.x - (xMiddle * 2));

    translate([- 15, 2, hotShoeHeightClearance * 2]) rotate([0, 0, 90])mangopi_mqquad_vertical_bracket();
}

module mangopi_mqquad_vertical_bracket() {
    middlePoint = IntersectionOfLines(feet[0], feet[3], feet[2], feet[1]);
    //    translate([0, 0, hotShoeHeightClearance - baseHeight]) translate(- middlePoint)vertical_bracket(feet, holeSize,
    //baseSize, baseHeight, totalHeight, linkThickness, linkHeight);
    translate([0, 0, hotShoeHeightClearance - baseHeight]) translate(- middlePoint)vertical_bracket(feet, holeSize,
    baseSize, baseHeight, totalHeight, linkThickness, linkHeight, true);
}

module mangopi_mqquad_hotShoe_adapter_base() {
    hotshoe_adapter(1);
    color("silver") hotshoe_adapter(hotShoeHeightClearance, true);
}

module mangopi_mqquad_screw_on_plank() {
    union() {
        mangopi_mqquad_bracket();
        earsForScrewingIntoAPlank(feet, linkThickness, linkHeight, baseSize, baseHeight, totalHeight);
    }
}