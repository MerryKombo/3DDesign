...
include <../module.scad>
include <../switch-module/8-ports-fast-ethernet-switch-dimensions.scad>
include <../../boards/8-ports-fast-ethernet-switch.scad>

eightPortsFastEthernetSwitchModule(true);
//eightPortsFastEthernetSwitchPCB(true);

module eightPortsFastEthernetSwitchModule(displayPCB = false) {
    union() {
        basicModule(eightPortsFastEthernetSwitchModuleWidth, eightPortsFastEthernetSwitchModuleLength, moduleHeight,
        pinsPath =
        false, nutRecess = false, rearDovetails = false, frontDovetails = true);
        xMove = (eightPortsFastEthernetSwitchModuleWidth - eightPortsFastEthernetSwitchWidth - wallThickness * 2) / 2;
        yMove = (eightPortsFastEthernetSwitchModuleLength - eightPortsFastEthernetSwitchLength - (rodSurroundingDiameter
            +
            surroundingDiameter) * 2) / 2;
        translate([wallThickness + xMove, rodSurroundingDiameter + surroundingDiameter + yMove, 0])
            eightPortsFastEthernetSwitchPCB(displayPCB);
    }
}

module eightPortsFastEthernetSwitchPCB(displayPCB = false) {
   translate([eightPortsFastEthernetSwitchWidth, 0, 0])
        rotate([0, 0, 90])
            union() {
                translate([0, 0, totalHeight])
                    if (displayPCB) {
                        color("#296E01") difference() {
                            cube([eightPortsFastEthernetSwitchLength, eightPortsFastEthernetSwitchWidth, 1.6]);
                            translate([topHoleDistanceFromEdgeX2, topHoleDistanceFromEdgeY, 0])
                                for (currentPoint = feet) {
                                    translate([currentPoint.x, currentPoint.y, - moduleHeight / 2]) cylinder(d =
                                    eightPortsFastEthernetSwitchHolesDiameter, h = moduleHeight, $fn = 100);
                                }
                        }
                    }
                eightPortsFastEthernetSwitchPCBFeet();
            }
}

module eightPortsFastEthernetSwitchPCBFeet() {
    translate([eightPortsFastEthernetSwitchDistanceFromEdgeX, eightPortsFastEthernetSwitchDistanceFromEdgeY, 0])
        difference() {
            union() {
...
module threeByThree() {
    echo("Will try to link those feet ", new_feet);
