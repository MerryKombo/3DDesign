...
module vertical_bracket(feet, holeSize, baseSize, baseHeight, totalHeight, linkThickness, linkHeight) {
    /*rotate(a = [-90, -90, 0])*/ union() {
        ...
        bracket_middle_reinforcement(feet, linkThickness, linkHeight);
    }
}

module bracket_middle_reinforcement(feet, linkThickness, linkHeight) {
    middlePoint = IntersectionOfLines(feet[0], feet[3], feet[2], feet[1]);
    topPoint = [max(feet[0][0], feet[1][0], feet[2][0], feet[3][0]), middlePoint.y];
    bottomPoint = [min(feet[0][0], feet[1][0], feet[2][0], feet[3][0]), middlePoint.y];
...
module mesh_link(points, thickness, height, number) {
    ...
    meshColor = xMove == 0 && yMove == 0 ? "white": "black";
...
module link_everyone(points, thickness, height) {
    ...
    for (idx = [0 : len(allLinks) - 1]) {
        ...
        echo("P is now: ", allLinks[idx]);
        mesh_link(allLinks[idx], thickness, height, idx);
    }
}
