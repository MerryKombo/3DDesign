    ...
    difference() {
        union() {
            color("Olive") hull() {
                ...
                color("grey") translate([0, 0, (totalHeight - baseHeight) / 2]) cylinder(r = footSize / 2, h =
                baseHeight, $fn = 100);
            }
            color("white") translate([0, 0, totalHeight - baseHeight]) cylinder(r = footSize / 2, h = baseHeight, $fn =
            100);
        }
...
module bracket_link(points, thickness, height) {
    ...
    union() {
        color("LightSteelBlue") translate([points[0].x, points[0].y, height * 2]) rotate([270, 0, - atan]) translate([0,
            0, holeSize / 2]) cylinder(h = length, r = thickness / 2, center = false, $fn = 100);
        color("OliveDrab") translate([points[0].x + firstSide + gap, points[0].y, height * 2]) rotate([270, 0, atan])
            translate([0, 0, holeSize / 2]) cylinder(h = length, r = thickness / 2, center = false, $fn = 100);
    }
}
...
module base_link(feet, feet2, thickness, height) {
    ...
    atan = atan(firstSide / secondSide);
    union() {
        color("LightSteelBlue") translate([feet2[0].x, feet2[0].y, totalHeight2 - thickness / 2]) rotate([270, 0, - atan
            ]) translate([0, 0
            , holeSize / 2]) cylinder(h = length, r = thickness / 2, center = false, $fn = 100);
        color("OliveDrab") translate([firstSide + gap + feet2[0].x, feet2[0].y, totalHeight2 - thickness / 2]) rotate([
            270, 0, atan])
            translate([0, 0, holeSize / 2]) cylinder(h = length, r = thickness / 2, center = false, $fn = 100);
    }
...
module topToBottomSupport1(feet, feet2, firstSide, gap, height, thickness) {
    ...
    if (feet2[1][0] - feet[0][0] > feet2[1][0] - feet[1][0]) {
        color("black") translate([firstSide + gap, 0, totalHeight2 - thickness / 2]) triangleRotation([feet2[1], feet[1]
            ]) rotate([270, 0, 90]) translate([0, 0, holeSize / 2]) cylinder(h = feet2[1][0] - (feet[1][0] + holeSize),
        r = thickness / 2, center = false, $fn = 100);
    } else {
        color("red") translate([firstSide + gap, 0, totalHeight2 - thickness / 2])  rotate([270, 0, 90])  translate([0,
            0, holeSize / 2]) cylinder(h = feet2[1][0] - feet[0][0], r = thickness / 2, center = false, $fn = 100);
    }
}
...
module topToBottomSupport2(feet, feet2, firstSide, gap, height, thickness) {
    ...
    if (feet2[3][0] - feet[2][0] > feet2[3][0] - feet[3][0]) {
        ...
        color("purple") translate([feet[3][0] + linkSize + holeSize, feet[3][1] + thickness + holeSize, feet[3][2] +
            totalHeight2 - thickness / 2])  triangleRotation([feet2[3], feet[3]]) rotate([270, 0, 90]) translate([0, 0,
                holeSize / 2]) cylinder(h = linkSize, r = thickness / 2, center = false, $fn = 100);
    } else {
        color("red") translate([firstSide + gap, 0, totalHeight2 - thickness / 2])  rotate([270, 0, 90])  translate([0,
            0, holeSize / 2]) cylinder(h = feet2[3][0] - feet[2][0], r = thickness / 2, center = false, $fn = 100);
    }
}
...
module topToBottomSupport3(feet, feet2, firstSide, gap, height, thickness) {
    ...
    if (feet2[2][0] - feet[2][0] > feet2[3][0] - feet[2][0]) {
        ...
        color("silver") translate([feet[2][0] + linkSize + holeSize, feet[2][1] + thickness + holeSize, feet[2][2] +
            totalHeight2 - thickness / 2])  rotate([0, 0, 90 + rotationAngle([feet2[2], feet[2]])]) rotate([270, 0, 90])
            translate([0, 0, holeSize / 2]) cylinder(h = linkSize, r = thickness / 2, center = false, $fn = 100);
    } else {
        linkSize = abs(feet2[2][0] - (feet[2][0] + holeSize));
        ...
        color("BurlyWood") translate([feet[2][0] + linkSize + holeSize, feet[2][1] + thickness + holeSize, feet[2][2] +
            totalHeight2 - thickness / 2]) triangleRotation([feet[2], feet2[2]]) rotate([270, 0, 90]) translate([0, 0,
                holeSize / 2]) cylinder(h = linkSize, r = thickness / 2, center = false, $fn = 100);
    }
}
...
module topToBottomSupport4(feet, feet2, firstSide, gap, height, thickness) {
    ...
    if (feet2[2][0] - feet[3][0] > feet2[3][0] - feet[3][0]) {
        ...
        color("DodgerBlue") translate([feet[3][0] + linkSize, feet[3][1] + thickness + holeSize, feet[3][2] +
            totalHeight2 - thickness / 2])  rotate([0, 0, 90 + rotationAngle([feet2[2], feet[3]])]) rotate([270, 0, 90])
            translate([0, 0, holeSize / 2]) cylinder(h = linkSize, r = thickness / 2, center = false, $fn = 100);
    } else {
        ...
        echo("rotation angle is", rotationAngle);
        //rotationAngle = 21.0567;
        color("Seashell") translate([feet[3][0], feet[3][1], feet[3][2] + totalHeight2 - thickness / 2])
            triangleRotation([feet[3], feet2[2]]) rotate([270, 0, 90]) translate([0, 0, holeSize / 2]) cylinder(h =
            linkSize, r = thickness / 2, center = false, $fn = 100);
    }
}
